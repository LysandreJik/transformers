...
class CursesCLI:
    ...
    def get_checkpoint_mapping(self, model_name, state_dict):
        ...
        architecture = CheckpointManager(model_name, state_dict)
...
class CheckpointManager:
    class CheckpointLayer(OrderedDict):

        index = 0
        all_layers: List["CheckpointManager.CheckpointLayer"] = []
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.index = CheckpointManager.CheckpointLayer.index
            CheckpointManager.CheckpointLayer.all_layers.append(self)
            CheckpointManager.CheckpointLayer.index += 1

        def get_layers(self):
            return {key: value.get_layers() for key, value in self.items() if
                    type(value) is CheckpointManager.CheckpointLayer and value['enabled']}

    def __init__(self, model_name: str, state_dict: OrderedDict) -> str:
        ...
        def regroup(state_dict_keys: List[str]):
            ...
            layer = CheckpointManager.CheckpointLayer()
            ...
            for key, value in layer.items():
                if isinstance(value, list) and len(value) > 0:
                    if len(value) == 1:
                        if value[0] == '':
                            layer[key] = CheckpointManager.CheckpointLayer()
                            layer[key]["torch _buffer"] = CheckpointManager.CheckpointLayer()
                        elif value[0] == 'weight':
                            layer[key] = CheckpointManager.CheckpointLayer()
                            layer[key]["nn.Embeddings"] = CheckpointManager.CheckpointLayer()
                    elif len(value) == 2:
                        if value[0] in ('weight', 'bias') and value[1] in ('weight', 'bias'):
                            if 'LayerNorm' in key or 'layer_norm' in key:
                                layer[key] = CheckpointManager.CheckpointLayer()
                                layer[key]["torch.nn.LayerNorm"] = CheckpointManager.CheckpointLayer()
                            else:
                                layer[key] = CheckpointManager.CheckpointLayer()
                                layer[key]["torch.nn.Linear"] = CheckpointManager.CheckpointLayer()
                    else:
        ...
        self.num_layers = len(self.CheckpointLayer.all_layers)
    ...
    def to_str(layer, nesting_level=1):
        return "".join(
            [
                ...
                + "\n"
                + (CheckpointManager.to_str(layer[key], nesting_level=nesting_level + 1) if len(layer[key]) > 0 and layer[key]['enabled'] else "")
    ...
    def toggle_layer(self, index):
        ...
        for layer in self.layer_indices_to_skip:
            if layer <= index:
                index += 1
        is_enabled = self.CheckpointLayer.all_layers[index]['enabled']

        if is_enabled:
            # If the layer is currently enabled, disable all the layers inside it when folding it
            def recurrent_disable(layer: CheckpointManager.CheckpointLayer):
                for key, value in layer.items():
                    if type(value) == CheckpointManager.CheckpointLayer:
                        value['enabled'] = False
                        # Add the layer indices to the set
                        self.layer_indices_to_skip.add(value.index)
                        recurrent_disable(value)

            self.CheckpointLayer.all_layers[index]['enabled'] = False
            recurrent_disable(self.CheckpointLayer.all_layers[index])
        else:
            # If the layer is disabled, enable this specific layer
            self.CheckpointLayer.all_layers[index]['enabled'] = True
            for key, value in CheckpointManager.CheckpointLayer.all_layers[index].items():
                # Remove the layer indices from the set
                if type(value) == CheckpointManager.CheckpointLayer:
                    self.layer_indices_to_skip.remove(value.index)

    def get_layers(self):
        return self.CheckpointLayer.all_layers[0].get_layers()
...
class AddNewModelCommand(BaseTransformersCLICommand):
    ...
    def run(self):
        ...
        # authors = get_message("Who are the authors?", default="The HuggingFace team")
        # encoder_decoder = get_message("Is your model an encoder-decoder or simply an encoder?", choices=["Encoder-decoder", "Encoder"])
        # checkpoint = get_message("Do you have an checkpoint for your model (can be TF1, TF2 or PyTorch)?", choices=["Yes", "No"])
